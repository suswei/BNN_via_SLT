...
class RealNVP(nn.Module):
    def __init__(self, nets, nett, mask, d, trueRLCT):
        ...
        self.lmbdas = torch.nn.Parameter(torch.cat((torch.ones(1, 1)*trueRLCT, torch.rand(d-1, 1)+trueRLCT)), requires_grad=False)
        # self.ks = torch.nn.Parameter(torch.cat((torch.rand(1, 1)+0.5, torch.rand(d-1, 1)+0.5)), requires_grad=True)
        self.ks = torch.nn.Parameter(torch.ones(d, 1)/2, requires_grad=False) #decreasing k will increase variance
        self.betas = torch.nn.Parameter(torch.rand(d-1, 1)+trueRLCT, requires_grad=False)
