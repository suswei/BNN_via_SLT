...
class RealNVP(nn.Module):
    def __init__(self, nets, nett, mask, d, trueRLCT):
        ...
        self.ks = torch.nn.Parameter(torch.cat((torch.ones(1, 1)*10, torch.ones(d-1, 1)/4)), requires_grad=True) #decreasing k will increase variance
        self.betas = torch.nn.Parameter(torch.rand(d-1, 1)+10, requires_grad=True)
