...
class RealNVP(nn.Module):
    def __init__(self, nets, nett, mask, d, lmbdas_requires_grad = True, lmbda0=None, k0=None, beta0=None):
        ...
        if lmbda0 is not None and k0 is not None and beta0 is not None:
            ...
            self.lmbdas = torch.nn.Parameter(torch.cat((torch.ones(1, 1), torch.ones(d-1, 1)*lmbda0)), requires_grad=lmbdas_requires_grad)
            ...
            self.ks = torch.nn.Parameter(torch.ones(d, 1) * k0, requires_grad=lmbdas_requires_grad)
            ...
            self.betas = torch.nn.Parameter(torch.ones(d-1, 1)*beta0, requires_grad=True)
