...
class NetIVI(nn.Module):
    def __init__(self, dims):
        super(NetIVI, self).__init__()
        self.dim1 = dims[0]
        if len(dims)==1:
            self.dim2 = 1
        else:
...
def evaluate(variational_parameter):
    ...
    out = variational_parameter.netvi(variational_parameter.eps)
    if variational_parameter.netvi.dim2 != 1:
        out = out.view(variational_parameter.netvi.dim1, variational_parameter.netvi.dim2)
    return out
...
class IVariationalize(nn.Module):
    """ Build a Implicit Variational model over the model given as input.
    ...
    def _variationalize_module(self, dico, module, prefix, zero_mean,
        ...
        for name, p in paras:
            if p is None:
                ...
                self.model_ivi.append(dico[name].netvi)
